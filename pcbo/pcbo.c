...
/*
 * Modified by Ben Spitz, June 2025
 *
 * Modifications:
 * * Default verbosity level changed to 0.
 * * At verbosity level 0, the total number of concepts is printed.
 * * This count is tallied in an unsigned long long.
 * * A warning is printed if the number of attributes or objects exceeds the
...
void read_context(FILE *file)
{
  ...
  if (attributes >= sizeof(counttype) * 8 && objects >= sizeof(counttype) * 8)
  {
    fprintf(stderr, "WARNING: Too many attributes and/or objects! Integer overflow may occur; program result is not guaranteed correct.\n");
  }
...
void print_thread_info(int id, struct thread_stat stat)
{
  if (verbosity_level >= 2)
    fprintf(stderr,
            "(:proc %3i :closures %7llu :computed %7llu :queue-length %3llu)\n",
...
void print_initial_thread_info(int levels, struct thread_stat stat, int last_level_concepts)
{
  if (verbosity_level >= 2)
    fprintf(stderr,
            "(:proc %3i :closures %7llu :computed %7llu :levels %i :last-level-concepts %i)\n",
...
void find_all_intents(void)
{
  ...
  if (verbosity_level >= 3)
    fprintf(stderr, "INFO: total %llu closures\n", counts[0].closures);
  if (verbosity_level >= 3)
    fprintf(stderr, "INFO: total %llu concepts\n", counts[0].computed);
}
