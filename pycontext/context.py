...
class FormalContext:
    ...
    def complexity(self):
        ...
        Returns:
            The complexity.
        """
        inverted_context_matrix = np.ones_like(self.matrix) - self.matrix # Invert the matrix to count 1's as 0's and vice versa
        ...
        for i in range(num_rows):
            ...
            A[num_rows + num_cols + i, num_rows:num_rows+num_cols] = inverted_context_matrix[i, :]
        # The next num_cols rows of A have the following form:
        # col_j CONCAT (N-1) * e_j
        for j in range(num_cols):
            A[num_rows + num_cols + num_rows + j, num_rows + j] = N - 1
            A[num_rows + num_cols + num_rows + j, :num_rows] = inverted_context_matrix[:, j]
        # Bounds
        ...
        res = opt.milp(c=c, constraints=constraints, integrality=integrality, bounds=bounds)
